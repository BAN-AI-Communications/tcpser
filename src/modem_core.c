...
int get_new_cts_state(modem_config *cfg, int up) { return DCE_CL_CTS; }
...
int mdm_set_control_lines(modem_config *cfg) {
  ...
  LOG(LOG_INFO, "Control Lines: DSR:%d DCD:%d CTS:%d",
      ((state & DCE_CL_DSR) != 0 ? 1 : 0), ((state & DCE_CL_DCD) != 0 ? 1 : 0),
...
void mdm_send_response(int msg, modem_config *cfg) {
  ...
  if (cfg->send_responses == TRUE) {
    ...
    if (cfg->text_responses == TRUE) {
      ...
      mdm_write(cfg, (unsigned char *)mdm_responses[msg],
                strlen(mdm_responses[msg]));
    } else {
...
int mdm_listen(modem_config *cfg) { return line_listen(&cfg->line_data); }
...
int mdm_parse_cmd(modem_config *cfg) {
  ...
  while (TRUE != done) {
    ...
    case 'S':
      ...
      if (num < sizeof(cfg->s)) {
        cfg->s[num] =
            atoi(tmp); // TODO do not assume this is always a number...
        switch (num) {
        case S_REG_CR:
          cfg->crlf[0] = cfg->s[S_REG_CR];
          break;
 ...
 *
 * AaaaaaT works (rule is that the first A found is considered the A char, and
 * the trailing T has the same case, so this works
 *
 * abcdefat also works (first 'a' starts the sequence, next letters starts over,
 * and then the last 'a' is paired up with the 't'
 *
 * At and aT will not work (the modem uses AT to determine parity, but mixed
 * case messes with the algorithm
 *
 * If a sequence is started (AT sent) but then the user backspaces, the
 * backspace is echoed to the sender, but then a 'T' is sent if the user has
 * erased all of the command
 *
...
int mdm_handle_char(modem_config *cfg, unsigned char ch) {
  ...
  if (cfg->is_cmd_started == TRUE) { // we previously got an 'AT'
    ...
    if (((ch_raw & 0x5f) == 'T') &&
        ((cfg->first_ch & 0x20) == (ch_raw & 0x20))) {
      cfg->is_cmd_started = TRUE;
      dce_detect_parity(&cfg->dce_data, cfg->first_ch, ch);
      LOG(LOG_ALL,
          "'T' parsed in serial stream, switching to command parse mode");
    } else if (ch_raw == '/') {
...
int mdm_send_ring(modem_config *cfg) {
  ...
  if (cfg->is_cmd_mode == FALSE ||
      (cfg->s[S_REG_RINGS] != 0 && cfg->rings >= cfg->s[S_REG_RINGS])) {
    mdm_answer(cfg);
  }
  return 0;
}
...
int mdm_parse_data(modem_config *cfg, unsigned char *data, int len) {
  ...
  if (cfg->is_cmd_mode == TRUE) {
    ...
    if (cfg->pre_break_delay == TRUE) {
      for (i = 0; i < len; i++) {
        if (dce_strip_parity(&cfg->dce_data, data[i]) ==
            (unsigned char)cfg->s[S_REG_BREAK]) {
